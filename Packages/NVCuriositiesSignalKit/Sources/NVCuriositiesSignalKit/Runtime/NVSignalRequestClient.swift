...
public struct NVSignalRequestClient: Sendable {
    public let configuration: NVSignalConfiguration
    public var session: URLSession

    public init(configuration: NVSignalConfiguration, session: URLSession? = nil) {
    ...
    public func loadDecision() async throws -> NVSignalLaunchDecision {
        try await loadDecision(preferredLanguage: Self.defaultLanguageCode())
    }

    public func loadDecision(preferredLanguage: String) async throws -> NVSignalLaunchDecision {
        ...
        return NVSignalLaunchDecision(
            enabled: true,
            url: Self.resolvedDestinationURL(base: url, preferredLanguage: preferredLanguage)
        )
    }

    public func makeLaunchRequest(payload: NVSignalLaunchPayload) throws -> URLRequest {
        var request = URLRequest(url: configuration.signalCheckURL)
        request.httpMethod = "POST"
        request.timeoutInterval = configuration.requestTimeout
        request.setValue("application/json", forHTTPHeaderField: "Content-Type")
        request.setValue("application/json", forHTTPHeaderField: "Accept")
        request.setValue("Bearer \(configuration.signalToken)", forHTTPHeaderField: "Authorization")
        request.setValue(configuration.signalToken, forHTTPHeaderField: "X-Analytics-Token")
    ...
    ) -> NVSignalLaunchPayload {
        let bundle = Bundle.main
        return NVSignalLaunchPayload(
            event: "app_open",
    ...
    private func send(request: URLRequest) async throws -> NVSignalLaunchDecision {
        ...
        return try JSONDecoder().decode(NVSignalLaunchDecision.self, from: data)
    }
