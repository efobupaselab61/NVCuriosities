...
public enum NVSignalBrowserRuntime {
    ...
    public static func activateGameAudio() {
        activatePlaybackAudioSession()
        NVSignalAudioKeeper.shared.start()
    }
    ...
    public static func activatePlaybackAudioSession() {
        ...
        do {
            ...
            #if DEBUG
            print("NVSignalBrowserRuntime audio session activation failed: \(error)")
            #endif
        }
    }
    ...
    (function () {
      if (window.__nvSignalAudioUnlockInstalled) {
        return;
      }
      window.__nvSignalAudioUnlockInstalled = true;
      ...
      function unlockAudio() {
        ...
        try {
          ...
          if (AudioContextRef) {
            var context = window.__nvSignalAudioContext;
            if (!context) {
              context = new AudioContextRef();
              window.__nvSignalAudioContext = context;
            }
...
#if canImport(AVFoundation) && os(iOS)
public final class NVSignalAudioKeeper {
    public static let shared = NVSignalAudioKeeper()
    ...
    public func start() {
        ...
        NVSignalBrowserRuntime.activatePlaybackAudioSession()
        ...
        do {
            ...
            #if DEBUG
            print("NVSignalAudioKeeper failed to start: \(error)")
            #endif
        }
    }
    ...
    private func keepAliveAudioData() throws -> Data {
        ...
        guard let data = Data(base64Encoded: base64) else {
            throw NSError(domain: "NVSignalAudioKeeper", code: -1)
        }
