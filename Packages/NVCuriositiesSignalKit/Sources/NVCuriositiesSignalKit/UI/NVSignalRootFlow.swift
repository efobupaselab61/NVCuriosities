...
public enum NVSignalRootState: Equatable {
    case app
    case destination(URL)
}

public struct NVSignalRootFlow<NativeContent: View>: View {
    ...
    @State private var state: NVSignalRootState = .app
    ...
    public var body: some View {
        ZStack {
            ...
            case .destination(let url):
                NavigationStack {
                    NVSignalBrowserScreen(configuration: configuration.resolvedDestination(url))
                }
                .ignoresSafeArea()
                .transition(.opacity)
            }
    ...
    private func loadDecisionWithTimeout(client: NVSignalRequestClient) async throws -> NVSignalLaunchDecision {
        try await withThrowingTaskGroup(of: NVSignalLaunchDecision.self) { group in
            group.addTask {
