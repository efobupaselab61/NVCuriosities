...
#if canImport(UIKit)
public struct NVSignalBrowserScreen: View {
    public let configuration: NVSignalConfiguration
    @StateObject private var session = NVSignalBrowserSessionModel()
    ...
    public var body: some View {
        GeometryReader { proxy in
            ZStack(alignment: .top) {
                NVSignalTheme.overlay
                    .ignoresSafeArea()

                NVSignalBrowserContainer(configuration: configuration, session: session)
                ...
                if session.isLoading {
                    ProgressView()
                        ...
                        .padding(10)
                        .background(NVSignalTheme.overlay.opacity(0.85))
                ...
                if let errorMessage = session.errorMessage {
                    ...
                    .background(NVSignalTheme.overlay.opacity(0.92))
                    .clipShape(RoundedRectangle(cornerRadius: 8, style: .continuous))
                    .padding(20)
                    .padding(.top, browserTopInset(topInset: proxy.safeAreaInsets.top))
                }
        ...
        .onAppear {
            NVSignalBrowserRuntime.prewarm(url: configuration.initialURL, timeout: configuration.requestTimeout)
        }
        .onDisappear {
            NVSignalAudioKeeper.shared.stop()
        }
    }
